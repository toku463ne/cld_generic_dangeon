...
func invade(m engine.Map, name string, seeds int, seed0 int64, ticks int) {
	...
	fmt.Printf("| 振った能力 | 成人到達率: 振った − 振らない | 死んだ成人1体あたりの子の数: 振った − 振らない | 振らない身体の成人到達率・子の数 |\n| --- | --- | --- | --- |\n")
	for _, inv := range invaders {
		var dRate, dKids, rate0, kids0 []float64
		for s := 0; s < seeds; s++ {
		...
		m0, _ := meanSE(rate0)
		k0, _ := meanSE(kids0)
		fmt.Printf("| %s | %s | %s | %.3f・%.2f |\n", inv.name, c(dRate, 4), c(dKids, 3), m0, k0)
	}
	fmt.Printf("\n`*` は差が標準誤差の2倍を超えるもの。\n\n")
}
