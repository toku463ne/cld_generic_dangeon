...
標準出力に `MEASURE.md` の形式のレポート（§2〜§9 と補助の表）を出す。tick ごとの表は、生存しているシードだけの平均と生存シード数を出し、全シードが下限を割ったあとの行は省く。§1 概要は人が書く。シードごとの実行時間は標準エラーに出す。保存則の検算が合わない tick があれば、その場で止まる。
